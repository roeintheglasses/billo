...
export const calculateCategoryTotal = (
  ...
  return categories.reduce((categoryTotal, category) => {
    ...
    const subscriptionTotal = category.subscriptions.reduce((subTotal, subscription) => {
      return subTotal + calculateSubscriptionTotal(
...
export const extractAllTransactions = (
  ...
  return userProfile.subscriptions.flatMap(subscription => 
...
export const calculateAverageMonthlySpending = (
  ...
  // Calculate average
  const totalSpending = sortedMonths.reduce(
  ...
  return subscriptions.reduce(
    (result, subscription) => {
      ...
      if (transactions.length >= 2) {
        ...
        const daysBetween = Math.round((secondDate.getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24));
