...
const PATTERN_REGISTRY: PatternDefinition[] = [
  ...
  {
    pattern:
      /(?:we've|we\s+have)\s+charged\s+your\s+(?:card|account|payment\s+method)\s+([£$€]?[\d,.]+)/i,
    type: PatternType.PAYMENT_CONFIRMATION,
    score: 85,
    extractors: { price: true },
  },
  ...
  {
    pattern:
      /your\s+subscription\s+(?:to\s+)?([A-Za-z0-9\s]+)?\s+has\s+(?:been\s+)?(?:confirmed|activated|started)/i,
    type: PatternType.SUBSCRIPTION_CONFIRMATION,
    score: 85,
    extractors: { serviceName: true },
  },
  ...
  {
    pattern:
      /your\s+(?:free\s+)?trial\s+(?:period\s+)?(?:for\s+)?([A-Za-z0-9\s]+)?\s+(?:will|is\s+about\s+to)\s+end/i,
  ...
  {
    pattern:
      /your\s+(?:free\s+)?trial\s+(?:period\s+)?ends\s+(?:on\s+)?(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})/i,
    type: PatternType.TRIAL_ENDING,
    score: 85,
  ...
  {
    pattern:
      /(?:after|once)\s+your\s+trial\s+ends\s+(?:on\s+)?(?:\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})?,\s+you\s+will\s+be\s+charged\s+([£$€]?[\d,.]+)/i,
    type: PatternType.TRIAL_ENDING,
    score: 90,
    extractors: { price: true },
  },
  {
    pattern:
      /reminder:?\s+your\s+(?:free\s+)?trial\s+(?:of\s+)?([A-Za-z0-9\s]+)?\s+is\s+ending\s+soon/i,
    type: PatternType.TRIAL_ENDING,
    score: 85,
    extractors: { serviceName: true },
  },
  ...
  {
    pattern:
      /your\s+(?:subscription|membership)\s+(?:to\s+)?([A-Za-z0-9\s]+)?\s+will\s+(?:auto[- ]?)?renew/i,
  ...
  {
    pattern:
      /your\s+(?:subscription|membership|plan)\s+(?:to\s+)?([A-Za-z0-9\s]+)?\s+has\s+been\s+renewed/i,
    type: PatternType.RENEWAL_NOTICE,
    score: 90,
    extractors: { serviceName: true },
  },
  ...
  {
    pattern:
      /we'll\s+automatically\s+bill\s+you\s+([£$€]?[\d,.]+)\s+(?:on|every)\s+(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})/i,
    type: PatternType.RENEWAL_NOTICE,
    score: 90,
  ...
  {
    pattern:
      /your\s+(?:subscription|membership|plan)\s+price\s+will\s+change\s+from\s+([£$€]?[\d,.]+)\s+to\s+([£$€]?[\d,.]+)/i,
    type: PatternType.PRICE_CHANGE,
    score: 95,
    extractors: { price: true },
  },
  ...
  {
    pattern:
      /your\s+(?:subscription|membership)\s+(?:to\s+)?([A-Za-z0-9\s]+)?\s+has\s+been\s+canceled/i,
  ...
  {
    pattern:
      /we've\s+received\s+your\s+request\s+to\s+cancel\s+your\s+([A-Za-z0-9\s]+)\s+subscription/i,
    type: PatternType.CANCELLATION,
    score: 90,
    extractors: { serviceName: true },
  },
...
const BILLING_CYCLE_PATTERNS = [
  ...
  {
    pattern: /yearly|per year|\/year|each year|a year|every year|annual|annually/i,
    cycle: 'yearly',
  },
  { pattern: /weekly|per week|\/week|each week|a week|every week/i, cycle: 'weekly' },
  {
    pattern: /quarterly|per quarter|\/quarter|each quarter|a quarter|every quarter|every 3 months/i,
    cycle: 'quarterly',
  },
...
export const analyzeSubscriptionText = (text: string, sender: string): PatternMatchResult => {
  ...
  if (advancedExtraction.service) {
    ...
    if (!result.matched && result.patternType === undefined) {
      ...
      if (text.toLowerCase().includes('renew') || text.toLowerCase().includes('next bill')) {
        ...
        (text.toLowerCase().includes('end') || text.toLowerCase().includes('expir'))
      ) {
        ...
        (text.toLowerCase().includes('change') || text.toLowerCase().includes('increas'))
      ) {
  ...
  if (advancedExtraction.date) {
    ...
    if (
      ...
      result.extractedData.nextBillingDate = advancedExtraction.date.date
        .toISOString()
        .split('T')[0];
    }
  }
