...
export function extractAmount(text: string): NormalizedAmount | null {
  ...
  const explicitPatterns = [
    // $XX.XX or $XX
    { pattern: /(?:^|\s)([$€£¥₹₩₽₺₴₱₦฿])\s*(\d+(?:[.,]\d{1,2})?)/i, confidence: 0.9, symbolIndex: 1, valueIndex: 2 },
    
    // XX.XX$ or XX€ (symbol after number)
    { pattern: /(?:^|\s)(\d+(?:[.,]\d{1,2})?)\s*([$€£¥₹₩₽₺₴₱₦฿])(?:\s|$)/i, confidence: 0.85, symbolIndex: 2, valueIndex: 1 },
    
    // Multi-character currency symbols (R$, C$, etc.)
    { pattern: /(?:^|\s)(R\$|C\$|A\$|NZ\$|HK\$|S\$|ر\.س|د\.إ|kr|Kč|zł|RM)\s*(\d+(?:[.,]\d{1,2})?)/i, confidence: 0.85, symbolIndex: 1, valueIndex: 2 },
    
    // XX.XX USD or XX USD (code after)
    { pattern: /(?:^|\s)(\d+(?:[.,]\d{1,2})?)\s*(USD|EUR|GBP|JPY|INR|KRW|BRL|SEK|CAD|AUD|CHF|NZD|RUB|THB|TRY|SAR|AED|UAH|PLN|CZK|MYR|PHP|NGN)(?:\s|$)/i, confidence: 0.85, symbolIndex: 2, valueIndex: 1, isCode: true },
    
    // USD XX.XX or USD XX (code before)
    { pattern: /(?:^|\s)(USD|EUR|GBP|JPY|INR|KRW|BRL|SEK|CAD|AUD|CHF|NZD|RUB|THB|TRY|SAR|AED|UAH|PLN|CZK|MYR|PHP|NGN)\s*(\d+(?:[.,]\d{1,2})?)(?:\s|$)/i, confidence: 0.85, symbolIndex: 1, valueIndex: 2, isCode: true }
  ];
  
  // Currency with prepositions or labels
  const contextualPatterns = [
    // "for $XX.XX" or "of $XX.XX"
    { pattern: /(?:for|of)\s+([$€£¥₹₩₽₺₴₱₦฿])\s*(\d+(?:[.,]\d{1,2})?)/i, confidence: 0.85, symbolIndex: 1, valueIndex: 2 },
    
    // "for XX.XX USD" or "of XX.XX USD"
    { pattern: /(?:for|of)\s+(\d+(?:[.,]\d{1,2})?)\s*(USD|EUR|GBP|JPY|INR|KRW|BRL|SEK|CAD|AUD|CHF|NZD|RUB|THB|TRY|SAR|AED|UAH|PLN|CZK|MYR|PHP|NGN)/i, confidence: 0.85, symbolIndex: 2, valueIndex: 1, isCode: true },
    
    // "amount: $XX.XX" or "price: $XX.XX"
    { pattern: /(?:amount|price|cost|fee|charge|payment):\s*([$€£¥₹₩₽₺₴₱₦฿])\s*(\d+(?:[.,]\d{1,2})?)/i, confidence: 0.9, symbolIndex: 1, valueIndex: 2 },
    
    // "amount: XX.XX USD" or "price: XX.XX USD"
    { pattern: /(?:amount|price|cost|fee|charge|payment):\s*(\d+(?:[.,]\d{1,2})?)\s*(USD|EUR|GBP|JPY|INR|KRW|BRL|SEK|CAD|AUD|CHF|NZD|RUB|THB|TRY|SAR|AED|UAH|PLN|CZK|MYR|PHP|NGN)/i, confidence: 0.9, symbolIndex: 2, valueIndex: 1, isCode: true },
    
    // "payment/charge of $XX.XX"
    { pattern: /(?:payment|charge|fee|subscription|bill)[^\n]*?(?:of|for)\s+([$€£¥₹₩₽₺₴₱₦฿])\s*(\d+(?:[.,]\d{1,2})?)/i, confidence: 0.9, symbolIndex: 1, valueIndex: 2 },
    
    // "payment/charge of XX.XX USD" 
    { pattern: /(?:payment|charge|fee|subscription|bill)[^\n]*?(?:of|for)\s+(\d+(?:[.,]\d{1,2})?)\s*(USD|EUR|GBP|JPY|INR|KRW|BRL|SEK|CAD|AUD|CHF|NZD|RUB|THB|TRY|SAR|AED|UAH|PLN|CZK|MYR|PHP|NGN)/i, confidence: 0.9, symbolIndex: 2, valueIndex: 1, isCode: true }
  ];
  
  // Currency with natural language names
  const naturalLanguagePatterns = [
    // "XX.XX dollars" or "XX dollars"
    { pattern: /(\d+(?:[.,]\d{1,2})?)\s+(dollars|euros|pounds|yen|rupees|won|reais|kronor|francs|baht|lira|riyal|dirhams|hryvnia|zloty|koruna|ringgit|peso|naira)(?:\s|$)/i, confidence: 0.8, nameIndex: 2, valueIndex: 1 },
    
    // "XX.XX US dollars" or "XX US dollars"
    { pattern: /(\d+(?:[.,]\d{1,2})?)\s+(?:US|canadian|australian|new zealand)\s+(dollars|euros|pounds)(?:\s|$)/i, confidence: 0.85, complexName: true, valueIndex: 1 }
  ];
  
  // Billing cycle with amount patterns
  const billingPatterns = [
    // "XX.XX/month" or "XX.XX per month"
    { pattern: /(\d+(?:[.,]\d{1,2})?)\s*(?:\/|\s+per\s+)(month|mo|year|yr|week|wk|day)(?:\s|$)/i, confidence: 0.8, cycleIndex: 2, valueIndex: 1 },
    
    // "$XX.XX/month" or "$XX.XX per month"
    { pattern: /([$€£¥₹₩₽₺₴₱₦฿])\s*(\d+(?:[.,]\d{1,2})?)\s*(?:\/|\s+per\s+)(month|mo|year|yr|week|wk|day)(?:\s|$)/i, confidence: 0.85, symbolIndex: 1, valueIndex: 2, cycleIndex: 3 }
  ];
...
export function extractServiceName(text: string, sender?: string): ExtractedService | null {
  ...
  if (sender) {
    ...
    if (sender.includes('@')) {
      ...
      if (domainParts.length >= 2) {
        ...
        if (capitalizedName.length > 2) { // Avoid single-letter names
          return {
            name: companyName,
      ...
      if (normalizedSender.length > 2) {
        ...
        let cleanedSender = normalizedSender
          .replace(/^sms-/, '')
          .replace(/^txt-/, '')
          .replace(/^info-/, '')
          .replace(/^alert-/, '');
  ...
  const servicePatterns = [
    ...
    { pattern: /(?:from|by)\s+([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})(?:\s|:|,|\.|$)/i, confidence: 0.85 },
    
    // "Company subscription" or "Company membership"
    { pattern: /([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})\s+(?:subscription|membership|plan|service)(?:\s|:|,|\.|$)/i, confidence: 0.85 },
    
    // "subscription to Company" or "membership to Company"
    { pattern: /(?:subscription|membership|payment)\s+(?:to|for|from)\s+([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})(?:\s|:|,|\.|$)/i, confidence: 0.85 },
    
    // "your Company plan" or "your Company account"
    { pattern: /your\s+([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})\s+(?:plan|account|subscription|membership|service)(?:\s|:|,|\.|$)/i, confidence: 0.85 },
    
    // "Company charges"
    { pattern: /([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})\s+(?:charges|fees|billing)(?:\s|:|,|\.|$)/i, confidence: 0.8 },
    
    // "thank you for subscribing to Company"
    { pattern: /thank\s+you\s+for\s+(?:subscribe|subscribing|your\s+subscription)\s+(?:to|with)\s+([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})(?:\s|:|,|\.|$)/i, confidence: 0.9 },
    
    // "welcome to Company"
    { pattern: /welcome\s+to\s+([A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,2})(?:\s|:|,|\.|$)/i, confidence: 0.85 }
  ];
  ...
  for (const { pattern, confidence } of servicePatterns) {
    ...
    if (match && match[1]) {
      ...
      for (const [alias, normalized] of Object.entries(SERVICE_ALIASES)) {
        if (extractedName.toLowerCase() === alias.toLowerCase() ||
  ...
  if (capitalizedWordMatch && capitalizedWordMatch[1]) {
    ...
    const commonWords = ['This', 'Your', 'Please', 'Thank', 'Information', 'Message', 'Subscription', 'Payment'];
  ...
  if (sender && sender.length > 2) {
    ...
    let cleanName = sender
      ...
      .replace(/^(sms|txt|info|alert|noreply|no-reply|billing|support|service|notification)[\s\-_:]+/i, '')
      .trim();
...
export function extractDate(text: string): ExtractedDate | null {
  ...
  const contextPatterns = [
    ...
    {
      pattern: /(?:ends|renews|expires|starts|begins|processes|renews|scheduled)(?:\s+on)?\s+(\w+\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?|\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4})/i,
      confidence: 0.85
    },
    // On date
    {
      pattern: /on\s+(\w+\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?|\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4})/i,
      confidence: 0.8
    },
    // Date at beginning of sentence with context
    {
      pattern: /(\w+\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?|\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4})(?:\s+is|,\s+your|\s+we)/i,
      confidence: 0.75
    }
  ];
  ...
  for (const { pattern, confidence } of contextPatterns) {
    ...
    if (match && match[1]) {
      ...
      try {
        ...
        const monthMatch = match[1].match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*/i);
        if (monthMatch) {
...
export function extractBillingCycle(text: string): ExtractedBillingCycle | null {
  ...
  if ((lowerText.includes('subscription') || lowerText.includes('recurring')) && 
...
function detectCurrencyFromContext(text: string): string | null {
  ...
  if (lowerText.includes('.uk') || 
      lowerText.includes('london') || 
      lowerText.includes('united kingdom')) {
    return 'GBP';
  }
  
  if (lowerText.includes('.eu') || 
      lowerText.includes('europe')) {
    return 'EUR';
  }
  
  if (lowerText.includes('.ca') || 
      lowerText.includes('canada')) {
    return 'CAD';
  }
  
  if (lowerText.includes('.au') || 
      lowerText.includes('australia')) {
    return 'AUD';
  }
  
  if (lowerText.includes('.nz') || 
      lowerText.includes('zealand')) {
    return 'NZD';
  }
  
  if (lowerText.includes('.in') || 
      lowerText.includes('india')) {
    return 'INR';
  }
  
  if (lowerText.includes('.jp') || 
      lowerText.includes('japan')) {
    return 'JPY';
  }
