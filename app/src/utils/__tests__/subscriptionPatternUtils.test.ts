...
describe('Subscription Pattern Utils', () => {
  ...
  describe('analyzeSubscriptionText', () => {
    ...
    it('should handle messages with mixed pattern types and prioritize by confidence', () => {
      ...
      const mixedMessage = 'Welcome to Netflix! Your payment of $9.99 has been processed. Your subscription is now active.';
  ...
  describe('extractServiceName', () => {
    it('should extract service name from sender and text', () => {
      expect(extractServiceName('Your Netflix subscription is active', 'updates@netflix.com')).toBe('Netflix');
      expect(extractServiceName('Payment to Spotify processed', 'random@example.com')).toBe('Spotify');
      expect(extractServiceName('Your Disney+ subscription renews tomorrow', 'billing@mail.com')).toBe('Disney+');
    });

    it('should extract service name using patterns', () => {
      expect(extractServiceName('Subscription to Paramount+ renewed', 'billing@mail.com')).toBe('Paramount+');
