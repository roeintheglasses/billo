...
describe('Subscription Data Extractor', () => {
  describe('extractAmount', () => {
    ...
    it('should extract amounts with currency codes', () => {
      ...
      expect(extractAmount('Charge of 29.90 BRL to your account')?.value).toBe(29.90);
    ...
    it('should provide confidence scores for different patterns', () => {
      expect(extractAmount('Your payment of $9.99 was processed')?.confidence).toBeGreaterThanOrEqual(0.8);
    ...
    it('should handle international formatting with commas', () => {
      expect(extractAmount('Your payment of €9,99 was processed')?.value).toBe(9.99);
      expect(extractAmount('Subscription fee of 1.299,00 EUR')?.value).toBe(1299.00);
    });
  });

  describe('extractServiceName', () => {
    it('should extract known service names', () => {
      expect(extractServiceName('Your Netflix subscription was renewed')?.normalizedName).toBe('Netflix');
      expect(extractServiceName('Payment to Spotify processed')?.normalizedName).toBe('Spotify');
      expect(extractServiceName('Your Disney+ subscription is active')?.normalizedName).toBe('Disney+');
    });

    it('should handle service name variations and aliases', () => {
      expect(extractServiceName('Your Prime subscription was renewed')?.normalizedName).toBe('Amazon Prime');
      expect(extractServiceName('Payment to Disney Plus processed')?.normalizedName).toBe('Disney+');
      expect(extractServiceName('Your Apple One subscription is active')?.normalizedName).toBe('Apple One');
      
      // Test new aliases
      expect(extractServiceName('Your Google One storage has been renewed')?.normalizedName).toBe('Google One');
      expect(extractServiceName('Payment to Audible processed')?.normalizedName).toBe('Audible');
      expect(extractServiceName('Your NordVPN subscription is active')?.normalizedName).toBe('NordVPN');
    });

    it('should extract service names from sender information', () => {
      expect(extractServiceName('Your subscription was renewed', 'noreply@netflix.com')?.normalizedName).toBe('Netflix');
      expect(extractServiceName('Payment processed', 'billing@spotify.com')?.normalizedName).toBe('Spotify');
      
      // Test new sender patterns
      expect(extractServiceName('Your subscription has been renewed', 'payments@disneyplus.com')?.normalizedName).toBe('Disney+');
      expect(extractServiceName('We charged your account', 'billing@hulu.com')?.normalizedName).toBe('Hulu');
      expect(extractServiceName('Payment confirmation', 'no-reply@youtube.com')?.normalizedName).toBe('YouTube Premium');
    });

    it('should extract service names using patterns', () => {
      expect(extractServiceName('Subscription to Paramount+ renewed')?.normalizedName).toBe('Paramount+');
      ...
      // Test new patterns
      expect(extractServiceName('Thank you for subscribing to Canva Pro')?.normalizedName).toBe('Canva Pro');
      expect(extractServiceName('Welcome to Dropbox Plus')?.normalizedName).toBe('Dropbox Plus');
      expect(extractServiceName('Your ExpressVPN charge has been processed')?.normalizedName).toBe('ExpressVPN');
    });

    it('should clean up sender information when used as fallback', () => {
      const result = extractServiceName('Your subscription was renewed', 'billing.info@unknown-service.com');
      ...
      const nestedSender = extractServiceName('Your subscription was renewed', 'noreply.billing@myservice.company.co.uk');
      expect(nestedSender?.normalizedName).toBeDefined();
    });

    it('should provide confidence scores for different extraction methods', () => {
      expect(extractServiceName('Your Netflix subscription was renewed')?.confidence).toBeGreaterThanOrEqual(0.8);
      expect(extractServiceName('Payment for your subscription', 'unknown@example.com')?.confidence).toBeLessThan(0.8);
      
      // Test confidence for different methods
      const directMatch = extractServiceName('Your Spotify subscription was renewed');
      const senderMatch = extractServiceName('Your subscription was renewed', 'billing@spotify.com');
  ...
  describe('extractBillingCycle', () => {
    ...
    it('should provide confidence scores for billing cycles', () => {
      expect(extractBillingCycle('Your monthly subscription')?.confidence).toBeGreaterThanOrEqual(0.8);
  ...
  describe('extractSubscriptionData', () => {
    ...
    it('should calculate higher confidence when multiple fields are extracted', () => {
      const fullText = 'Your Netflix monthly subscription payment of $9.99 was processed on 05/15/2023';
    ...
    it('should boost confidence when critical fields are present', () => {
      ...
      expect(withCriticalResult.overallConfidence).toBeGreaterThan(withoutCriticalResult.overallConfidence);
    });
    
    it('should extract data from international formats', () => {
      const ukText = 'Your Spotify monthly subscription payment of £9.99 was processed on 15/05/2023';
    ...
    it('should extract data from complex real-world messages', () => {
      const complexText = 'Thank you for your Disney+ subscription! Your payment of $7.99 was successfully processed on May 2, 2023. Your next monthly payment will be charged on June 2, 2023.';
    ...
    it('should consider sender information when calculating confidence', () => {
      ...
      expect(withMatchingSender.overallConfidence).toBeGreaterThan(withNonMatchingSender.overallConfidence);
    });
  });
}); 