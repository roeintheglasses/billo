...
export const BasicInsights: React.FC<BasicInsightsProps> = ({
  ...
  const fetchInsights = async () => {
    try {
      ...
      const categoryMap: Record<string, number> = subscriptions.reduce(
      ...
      const mostFrequentCategory = mostFrequentCategoryId
        ? subscriptions.find((sub: Subscription) => sub.category_id === mostFrequentCategoryId)
            ?.category
        : null;
  ...
  if (loading) {
    return (
      <View
        style={[styles.container, { backgroundColor: colors.background?.primary || '#ffffff' }]}
      >
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { color: colors.text?.primary || '#000000' }]}>
            Loading insights...
          </Text>
        </View>
      </View>
    ...
    return (
      ...
      >
        ...
        <View style={styles.errorContainer}>
          ...
          >
            <Text style={[styles.retryButtonText, { color: colors.text?.inverted || '#ffffff' }]}>
              Retry
            </Text>
          </TouchableOpacity>
        </View>
    ...
    <View style={[styles.container, { backgroundColor: colors.background?.primary || '#ffffff' }]}>
      ...
      <View style={styles.insightsContainer}>
        ...
        {insights.highestCost && (
          ...
          >
            ...
            <View style={styles.insightContent}>
              <Text style={[styles.insightTitle, { color: colors.text?.primary || '#000000' }]}>
                Highest Cost
              </Text>
              <Text
                style={[styles.insightValue, { color: colors.text?.primary || '#000000' }]}
                numberOfLines={1}
              >
                {insights.highestCost.name}
              </Text>
              <Text
                style={[styles.insightSubvalue, { color: colors.text?.secondary || '#666666' }]}
              >
                {formatCurrency(
                  subscriptionService.normalizeAmountToMonthly(
                    insights.highestCost.amount,
                    insights.highestCost.billing_cycle
                  )
                )}
                /mo
              </Text>
        ...
        {insights.mostFrequentCategory &&
          insights.mostFrequentCategoryId &&
          insights.categoryMap && (
            <TouchableOpacity
              style={[
                styles.insightCard,
                {
              ...
              onPress={() =>
                handleInsightPress('mostFrequentCategory', insights.mostFrequentCategory)
              }
              accessible={true}
