 ...
 */
export const getAllSubscriptionsWithDetails = async (): Promise<SubscriptionWithCategoryAndTransactions[]> => {
  try {
    ...
    return data.map(subscription => {
      const subscriptionTransactions = allTransactions?.filter(
        t => t.subscription_id === subscription.id
      ) || [];
 ...
 */
export const getSpendingTimeSeries = async (months: number = 12): Promise<SpendingAnalytics['timeSeriesData']> => {
  try {
 ...
 */
export const getUserSubscriptionProfile = async (userId: string): Promise<UserWithSubscriptionsAndTransactions> => {
  try {
    // Get user data
