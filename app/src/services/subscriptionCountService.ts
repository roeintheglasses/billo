...
export const calculateSubscriptionCountByCategory = async (): Promise<CategoryCount[]> => {
  try {
    ...
    const totalSubscriptions = categoryCountData.reduce((sum, item) => sum + item.count, 0);

    // Calculate percentages
    return categoryCountData.map(item => ({
      ...item,
      percentage: totalSubscriptions > 0 ? Math.round((item.count / totalSubscriptions) * 100) : 0,
    }));
  } catch (error: any) {
